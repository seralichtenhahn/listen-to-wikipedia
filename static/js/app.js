function wp_action(data, svg_area, silent) {
  ...
  if (total_edits == 1) {
    $("#edit_counter").html(
      "You have listened to <span>" + total_edits + " edit</span>."
    );
  } else {
    $("#edit_counter").html(
      "You have listened to a total of <span>" +
        insert_comma(total_edits) +
        " edits</span>."
    );
  }
  var now = new Date();
  ...
  var circle_group = svg_area
    .append("g")
    .attr("transform", "translate(" + x + ", " + y + ")")
    .attr("fill", edit_color)
    .style("opacity", starting_opacity);

  var ring = circle_group
    .append("circle")
    .attr({ r: size + 20, stroke: "none" })
    .transition()
    .attr("r", size + 40)
    .style("opacity", 0)
    .ease(Math.sqrt)
  ...
  var circle_container = circle_group
    .append("a")
    .attr("xlink:href", data.url)
    .attr("target", "_blank")
    .attr("fill", svg_text_color);

  var triangle = circle_container
    .append("path")
    .classed(type, true)
    .attr(
      "d",
      "M0,-" + size + " L" + size + "," + size + " -" + size + "," + size + " Z"
    )
    .transition()
    .duration(max_life)
    .style("opacity", 0)
    ...
    if ("WebSocket" in window) {
      ...
      connection.onmessage = function (resp) {
        ...
        if (data.ns == "Main" || DEBUG) {
          if (
            !isNaN(data.change_size) &&
            (TAG_FILTERS.length == 0 ||
              $(TAG_FILTERS).filter(
                $.map(data.hashtags, function (i) {
            ...
            if (data.change_size < 0) {
              if (data.change_size == -1) {
                rc_str +=
                  " removed " + Math.abs(data.change_size) + " byte from";
              } else {
                rc_str +=
                  " removed " + Math.abs(data.change_size) + " bytes from";
              }
          ...
          data.page_title == "Special:Log/newusers" &&
          data.url != "byemail" &&
          s_welcome
        ) {
          ...
          var nu_str =
            '<a href="http://' +
            lid +
            ".wikipedia.org/w/index.php?title=User_talk:" +
            data.user +
...
function play_sound(size, type, volume) {
  ...
  var pitch =
    100 - Math.min(max_pitch, Math.log(size + log_used) / Math.log(log_used));
...
function newuser_action(data, lid, svg_area) {
  play_random_swell();
  var messages = [
    "Welcome to " + data.user + ", Wikipedia's newest user!",
  ...
  var user_container = user_group
    .append("a")
    .attr("xlink:href", user_link)
    .attr("target", "_blank");

  user_group.transition().delay(7000).remove();

  user_container.transition().delay(4000).style("opacity", 0).duration(3000);
...
function compute_and_update_epm(svg_area) {
  ...
  for (var i = 0; i < N; i++) {
    windows[i] = deltas.filter(
      (d) => d < (i + 1) * WINDOW_INTERVAL && d > i * WINDOW_INTERVAL
    ).length;
...
function update_epm(epm, svg_area) {
  if (!epm_text) {
    epm_container = svg_area
      .append("g")
      .attr("transform", "translate(0, " + (height - 25) + ")");

    var epm_box = epm_container
      .append("rect")
      .attr("fill", newuser_box_color)
      .attr("opacity", 0.5)
      .attr("width", 135)
